...
for idx in range(len(train_backdoor_triggers)):
    pre_data = {}
    with open(f'../data/prompts/train/rickrolling/train_data_{idx+1}.txt','r',encoding='utf-8') as fin:
        lines = fin.readlines()
    num=0
    for id in tqdm(range(len(lines))):
        ...
        line = lines[id]
        prompt = line.strip('\n')
        # if train_backdoor_triggers[idx] in prompt:
        prompt = [prompt]
        controller = AttentionStore()
        ...
        pre_data[id]=(images,length)

    np.save(f'../data/attention_maps/eval/rickrolling/train_data_{idx+1}.npy', pre_data)

# test attention maps 
for idx in range(len(test_backdoor_triggers)):
    pre_data = {}
    with open(f'../data/prompts/test/rickrolling/test_data_{idx+1}.txt','r',encoding='utf-8') as fin:
        lines = fin.readlines()
    num=0
    for id in tqdm(range(len(lines))):
        controller.reset()
    ...
    np.save(f'../data/attention_maps/eval/rickrolling/eval_data_{idx+1}.npy', pre_data)
